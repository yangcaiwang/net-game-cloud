...
import com.ycw.core.internal.db.sql.CREATEDATABASE;
import com.ycw.core.internal.db.sql.DELETEBYKEY;
import com.ycw.core.internal.db.sql.SELECT;
...
final public class DaoImpl implements IDao {
    ...
    public <T extends DBEntity> Boolean createTableIfNotExists(Class<T> entityType, String aliasName, String tableName) {
        try {
            Integer result = executeUpdate(aliasName, buildSQL(entityType, com.ycw.core.internal.db.constant.SQLType.CREATE_TABLE, tableName));
            return result >= 0;
        } catch (Exception e) {
            throw new RuntimeException(e.getMessage(), e);
        }
    ...
    public <T extends DBEntity> Long returnGeneratedKeys(T obj) {
        ...
        ResultSet rs = null;
        try {
            sql = buildSQL(entityType, com.ycw.core.internal.db.constant.SQLType.INSERT, tab);
            conn = open(aliasName);
            if (useInnodb())
                conn.setAutoCommit(false);
            pst = conn.prepareStatement(sql, PreparedStatement.RETURN_GENERATED_KEYS);
            obj.setFirstCreateTime(new Date());
            Wrapper.getInstance().wrap(pst, obj);
            pst.executeUpdate();
            if (useInnodb())
                conn.commit();
    ...
    public <T extends DBEntity> Boolean insert(Collection<T> c, Class<T> entityType, String aliasName, String tableName) {
        ...
        PreparedStatement pst = null;
        try {
            sql = buildSQL(entityType, com.ycw.core.internal.db.constant.SQLType.INSERT, tableName);
            conn = open(aliasName);
            if (useInnodb())
                conn.setAutoCommit(false);
            pst = conn.prepareStatement(sql);
    ...
    public <T extends DBEntity> Boolean insertOrUpdate(Collection<T> c, Class<T> entityType, String aliasName, String tableName) {
        ...
        PreparedStatement pst = null;
        try {
            sql = buildSQL(entityType, com.ycw.core.internal.db.constant.SQLType.INSERT_EXIST_KEY_UPDATE, tableName);
            conn = open(aliasName);
            if (useInnodb())
                conn.setAutoCommit(false);
            pst = conn.prepareStatement(sql);
            ...
            for (T obj : c) {
                ...
                for (Field field : fields) {
                    Wrapper.getInstance().wrap(pst, obj, field, i++);
                }
                for (Field field : fields) {
                    Wrapper.getInstance().wrap(pst, obj, field, i++);
                }
    ...
    public <T extends DBEntity> Boolean insertIfNotExists(Collection<T> c, Class<T> entityType, String aliasName, String tableName) {
        ...
        PreparedStatement pst = null;
        try {
            sql = buildSQL(entityType, com.ycw.core.internal.db.constant.SQLType.INSERT_IGNORE, tableName);
            conn = open(aliasName);
            if (useInnodb())
                conn.setAutoCommit(false);
            pst = conn.prepareStatement(sql);
    ...
    public <T extends DBEntity> Boolean replace(Collection<T> c, Class<T> entityType, String aliasName, String tableName) {
        ...
        PreparedStatement pst = null;
        try {
            sql = buildSQL(entityType, com.ycw.core.internal.db.constant.SQLType.REPLACE, tableName);
            conn = open(aliasName);
            if (useInnodb())
                conn.setAutoCommit(false);
            pst = conn.prepareStatement(sql);
    ...
    public <T extends DBEntity> Boolean delete(Class<T> entityType, Serializable... pks) {
        ...
        PreparedStatement pst = null;
        try {
            sql = buildSQL(entityType, com.ycw.core.internal.db.constant.SQLType.DELETE_BY_KEY, tab);
            conn = open(aliasName);
            if (useInnodb())
                conn.setAutoCommit(false);
            pst = conn.prepareStatement(sql);
            ...
            Wrapper.getInstance().wrap(pst, pks);
...
//            sql = buildSQL(entityType, SQLType.DELETEBYKEY, tab);
    ...
    public <T extends DBEntity> boolean delete(Collection<T> c, Class<T> entityType, String aliasName, String tableName) {
        ...
        PreparedStatement pst = null;
        try {
            sql = buildSQL(entityType, com.ycw.core.internal.db.constant.SQLType.DELETE_BY_KEY, tableName);
            conn = open(aliasName);
            if (useInnodb())
                conn.setAutoCommit(false);
            pst = conn.prepareStatement(sql);
            ...
            for (T obj : c) {
                ...
                Wrapper.getInstance().wrap(pst, keys);
                pst.addBatch();
                if (!isLogDb) {
                    getPreparedStatementSQL(pst);
                }
    ...
    public <T extends DBEntity> Boolean update(Collection<T> c, Class<T> entityType, String aliasName, String tableName) {
        ...
        PreparedStatement pst = null;
        try {
            sql = buildSQL(entityType, com.ycw.core.internal.db.constant.SQLType.UPDATE_BY_KEY, tableName);
            conn = open(aliasName);
            if (useInnodb())
                conn.setAutoCommit(false);
            pst = conn.prepareStatement(sql);
            ...
            for (T obj : c) {
                ...
                int j = Wrapper.getInstance().wrap(pst, obj, valueFields, 1);
                Wrapper.getInstance().wrap(pst, obj, keyFields, j);
                pst.addBatch();
                if (!isLogDb) {
                    getPreparedStatementSQL(pst);
                }
    ...
    public <T extends DBEntity> List<T> queryAll(Class<T> entityType, String aliasName, String tableName) {
        ...
        try {
            sql = buildSQL(entityType, com.ycw.core.internal.db.constant.SQLType.SELECT_ALL, tableName);
            conn = open(aliasName);
            pst = conn.prepareStatement(sql);
            rs = pst.executeQuery();
            while (rs != null && rs.next()) {
    ...
    public <T extends DBEntity> T query(Class<T> entityType, String aliasName, String tableName, Serializable... pks) {
        ...
        String sql = null;
        try {
            sql = buildSQL(entityType, com.ycw.core.internal.db.constant.SQLType.SELECT_BY_KEY, tableName);
            conn = open(aliasName);
            pst = conn.prepareStatement(sql);
            Wrapper.getInstance().wrap(pst, pks);
            rs = pst.executeQuery();
    ...
    public <T extends DBEntity> Map<String, T> queryAll(Class<T> entityType, String aliasName, String tableName, Serializable[]... pksArray) {
        ...
        try {
            sql = String.format(buildSQL(entityType, com.ycw.core.internal.db.constant.SQLType.SELECTOR, tableName), SELECT.BUILDKEYSOR(entityType, pksArray));
            conn = open(aliasName);
            pst = conn.prepareStatement(sql);
            rs = pst.executeQuery();
            while (rs != null && rs.next()) {
    ...
    public <T extends DBEntity> Map<String, T> queryAll(Class<T> entityType, String aliasName, String tableName, Collection<Long> keys) {
        ...
        Map<String, T> result = Maps.newHashMap();
        try {
            sql = String.format(buildSQL(entityType, com.ycw.core.internal.db.constant.SQLType.SELECT_IN, tableName), SELECT.BUILDKEYSIN(entityType, keys));
            conn = open(aliasName);
            pst = conn.prepareStatement(sql);
            rs = pst.executeQuery();
            while (rs != null && rs.next()) {
