...
public class PropertyConfig {
    ...
    /**
     * 获取路径前缀兼容linux和windows
     *
     * @return 路径前缀
     */
    public static String getPrefixPath() {
        if (System.getProperty("path.separator").equals(":")) {// 如果是Linux
            return "home/game/";
        } else {
     ...
     * @param path 绝对路径
     */
    public static Map<String, Object> loadYml(String path) throws IOException {
        // 创建Yaml实例
        Yaml yaml = new Yaml();
        InputStream in = new BufferedInputStream(Files.newInputStream(Paths.get(path)));
        // 解析YAML文件
    ...
    public static void modifyClusterYml(String clusterYmlPath, String key, int value) {
        try {
            ...
            Map<String, Object> ymlMap = loadYml(clusterYmlPath);
