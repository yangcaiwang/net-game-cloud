...
/**
 * 持久化日志数据滚表类型 如果有滚表,那么通过父类提供了快捷查询数据方法只能查当前最新生成那张表的数据,建议使用DaoImpl的api来操作数据库
 */
public enum RollType {
	...
	private static String suffix(RollType rollType, long t) {
		...
		case EVERY_WEEK_OF_YEAR:
			long time = t;
			int weekIndex = rollType == EVERY_WEEK_OF_MONTH ? DateUnit.weekOfMonth(time) : DateUnit.weekOfYear(time);
			String dateStr = formatter.format(new Date(time));
...
//
//		for (RollType rollType : values()) {
//			System.err.println(fixedTableName("t_test_tab", rollType));
//		}
//		System.err.println();
//		for (RollType rollType : values()) {
//			System.err.println(parsedTableName("t_test_tab", rollType,1672728099000L));
//		}
//	}
}
